...
class DistriControlnetPP(BaseModel):  # for Patch Parallelism
    def __init__(self, model: ControlNetModel, distri_config: DistriConfig):
        ...
        super(DistriControlnetPP, self).__init__(model, distri_config)
    ...
    def forward(
        ...
        # distri_config.use_cuda_graph = False
        if False and distri_config.use_cuda_graph and not record:
            ...
            if distri_config.world_size == 1:
                ...
                controlnet_cond = controlnet_cond[batch_idx : batch_idx + 1]
                ...
                if self.output_buffer is None:
                    self.output_buffer = torch.empty((b, c, h, w), device=sample.device, dtype=sample.dtype)
                if self.buffer_list is None:
                    # self.buffer_list = [torch.empty_like(output) for _ in range(distri_config.world_size)]
                    self.buffer_list = [torch.empty_like(sample) for _ in range(distri_config.world_size)]
                dist.all_gather(self.buffer_list, sample.contiguous(), async_op=False)
